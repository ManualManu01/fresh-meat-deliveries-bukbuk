...
import { ArrowLeft, Clock, CheckCircle, Truck, MapPin, Phone, User, Calendar, AlertCircle, ShoppingBag, Star, Package, ChefHat, Route, History } from 'lucide-react';
...
const OrderTracking = ({ onBack }: OrderTrackingProps) => {
  ...
  const { currentOrder, updateOrderStatus, orders } = useUserOrders();
  const { user } = useAuth();
  ...
  const trackingSteps = [
    { 
      ...
      details: 'Payment verified • Order #' + currentOrder?.orderId + ' created • Notification sent'
    },
    { 
      id: 1, 
      ...
      details: `Chef: ${currentOrder?.deliveryPartner?.name} • Est. 12-15 minutes • Extra fresh, premium cuts`
    },
    { 
      id: 2, 
      ...
      details: `Partner: ${currentOrder?.deliveryPartner?.name} • Vehicle: DL 01 AB 1234 • Distance: 2.5 km`
    },
    { 
      id: 3, 
      title: 'Delivered', 
  ...
  useEffect(() => {
    // Simulate order progression for current order
    if (currentOrder) {
      const progressTimer = setInterval(() => {
        setCurrentStep(prev => {
          ...
          const statusMap = ['confirmed', 'preparing', 'out_for_delivery', 'delivered'] as const;
          if (newStep < statusMap.length) {
            updateOrderStatus(currentOrder.orderId, statusMap[newStep]);
          }
  ...
  // If no current order and no order history, show different UI
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center">
              ...
              <h1 className="text-4xl font-bold bg-gradient-to-r from-pink-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent">
                {showHistory ? 'Order History' : 'Track Your Order'}
              </h1>
            </div>
          ...
          {showHistory ? (
            <OrderHistory onViewOrder={(order) => {
              SoundEffects.playClickSound();
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        <div>
                          <h3 className="font-semibold mb-3 text-white text-lg flex items-center gap-2">
                            ...
                            Order Items ({currentOrder.items.reduce((sum, item) => sum + item.quantity, 0)})
                          </h3>
                          <div className="space-y-3">
                            ...
                            <div className="border-t border-purple-500/20 pt-4">
                              ...
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <span className="text-white">₹{(currentOrder.total * 0.05).toFixed(2)}</span>
                              </div>
                              <div className="flex justify-between items-center border-t border-purple-500/20 pt-2">
                                ...
                                <span className="font-bold text-2xl bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent">₹{(currentOrder.total * 1.05).toFixed(2)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-purple-500/20">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-400">Expected: {currentOrder.estimatedDelivery?.toLocaleTimeString()}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-purple-500/20">
                              ...
                              <div>
                                ...
                                <p className="text-gray-300 text-sm">{currentOrder.phone}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-purple-500/20">
                              ...
                              <div>
                                ...
                                <p className="text-gray-300 text-sm">{currentOrder.orderTime?.toLocaleString()}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <Card className="bg-white/5 backdrop-blur-md border border-purple-500/20 shadow-2xl">
                      <CardContent className="p-6">
                        ...
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div className="flex-1">
                            <p className="font-medium text-white text-lg">{currentOrder.deliveryPartner?.name}</p>
                            <p className="text-sm text-gray-400">Delivery Partner</p>
                            <div className="flex items-center gap-2 mt-1">
                              {renderStars(currentOrder.deliveryPartner?.rating || 4.8)}
                              <span className="text-xs text-gray-400">({currentOrder.deliveryPartner?.deliveries} deliveries)</span>
                            </div>
                            <div className="flex items-center gap-1 mt-1">
                        ...
                        <div className="mb-4 p-3 bg-white/5 rounded-lg border border-purple-500/20">
                          <p className="text-sm text-gray-300">Vehicle: {currentOrder.deliveryPartner?.vehicleNumber}</p>
                        ...
                        >
                          ...
                          Call {currentOrder.deliveryPartner?.name}
                        </Button>
                        <Button 
                          variant="outline"
            ...
            // Show order history if no current order but have past orders
            <OrderHistory />
          )}
        </div>
      </div>
